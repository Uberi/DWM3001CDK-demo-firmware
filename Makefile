build: development-environment
	docker run -v "$$(pwd)":/project uberi/qorvo-nrf52833-board /usr/local/segger_embedded_studio_V7.32/bin/emBuild -config "Common" /project/DWM3001CDK-DW3_QM33_SDK_CLI-FreeRTOS.emProject

clean: development-environment
	docker run -v "$$(pwd)":/project uberi/qorvo-nrf52833-board /usr/local/segger_embedded_studio_V7.32/bin/emBuild -config "Common" -clean /project/DWM3001CDK-DW3_QM33_SDK_CLI-FreeRTOS.emProject
